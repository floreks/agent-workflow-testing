...
test("can delete a message", async ({ page }) => {
  ...
  const messageItem = page.locator("li", { hasText: message });
  await messageItem.getByRole("button", { name: "Delete" }).click();

  await expect(messageItem).toBeHidden({ timeout: 10_000 });
});
